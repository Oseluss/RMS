...
from algorithms.agents.agents import SoftmaxAgent, GaussianAgent
...
class PPOSoftmaxNN:
    ...
    def select_action2(self, state: np.ndarray) -> np.ndarray:
        with torch.no_grad():
            ...
            action = pi.sample().detach().flatten()
            action_logprob = pi.log_prob(action)
            if len(action_logprob.shape) > 1:
                action_logprob =  torch.sum(action_logprob, dim=1)
            else:
                action_logprob =  action_logprob
            action_logprob = torch.sum(pi.log_prob(action)).detach().flatten()
        #self.buffer.states.append(state)
        #self.buffer.actions.append(action)
        #self.buffer.logprobs.append(action_logprob)
...
class PPOGaussianNN:
    def __init__(
        ...
        self.policy = GaussianAgent(actor, critic, discretizer_actor, discretizer_critic,device)
        self.policy_old = GaussianAgent(actor_old, critic_old, discretizer_actor, discretizer_critic,device)
        self.policy_old.load_state_dict(self.policy.state_dict())
